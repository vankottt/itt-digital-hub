...
import { analysisRadiusForSettlement, analyzeSettlement } from './analysis'
...
function fixture(withData = true): RawGeodata {
  ...
  return {
    all: featureCollection([...buildings, ...landuse, ...roads, ...pois] as Feature<Geometry, OsmFeatureProperties>[]),
...
describe('анализ на населено място', () => {
  ...
  it('остава работоспособен при липса на сгради, улици и land-use', () => {
    ...
    expect(result.confidence.level).toBe('Ниска')
  ...
  it('използва близката жилищна зона, когато сградните данни са недостатъчни', () => {
    ...
    expect(result.boundaryReason).toContain('жилищна зона')
