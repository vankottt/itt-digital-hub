...
import { OrchestrationCardDiagram, WarrantyCardDiagram } from "./ProjectVisuals";

function Cover({ cover, locale }: { cover?: StoryCover; locale: Locale }) {
  if (cover?.kind === "diagram") {
    return (
      <div className="absolute inset-0 bg-marine" aria-hidden="true">
        {cover.diagram === "warranty-relation" ? <WarrantyCardDiagram locale={locale} /> : <OrchestrationCardDiagram locale={locale} />}
      </div>
    );
  }
...
export function ProjectTags({ tags }: { tags: readonly string[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {tags.map((tag) => (
        <span key={tag} className="rounded-full border border-line px-3 py-1 text-meta text-ink-2">
          {tag}
        </span>
      ))}
    </div>
...
export function StoryCard({
  ...
  return (
    <article className="h-full">
      <Link href={url} className="story-card group flex h-full flex-col no-underline outline-offset-4">
        <div className="relative isolate aspect-[16/9] overflow-hidden rounded-[1.25rem] bg-paper-2">
          ...
          <div
            className={cn(
              ...
              cover?.kind === "diagram"
                ? "bg-gradient-to-t from-marine/70 via-transparent to-transparent"
          ...
          <span className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-5 text-on-dark">
            <span className="font-sans text-small font-medium">{m.viewProject}</span>
