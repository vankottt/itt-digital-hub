...
import type { AdminLead } from '../server/admin-data'
...
export function AdminDashboard({ adminName, privacyReady, leads, kpis, funnel }: Props) {
  ...
  return (
    <main className="admin-page">
      <div className="page-topbar"><span>← Анализатор</span><strong>Анализ и квалификация</strong><span className="admin-identity">Администратор: {adminName}</span></div>
      <div className="admin-content">
        ...
        <section className="admin-card funnel-card"><div className="admin-section-title"><div><h2>Конференционна фуния</h2><p>Преминаване между ключовите стъпки.</p></div></div><div className="funnel-list">{funnel.map((item, index) => { const previous = index === 0 ? item.value : funnel[index - 1]?.value ?? 0; const conversion = previous ? item.value / previous * 100 : 0; const max = Math.max(1, funnel[0]?.value ?? 1); return <div className="funnel-row" key={item.label}><span>{item.label}</span><div className="funnel-bar"><i style={{ width: `${Math.max(3, item.value / max * 100)}%` }} /></div><strong>{item.value}</strong><em>{index === 0 ? '100%' : formatPercent(conversion)}</em></div> })}</div></section>
